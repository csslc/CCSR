...
def main() -> None:
    ...
    for file_path in list_image_files(args.input_list, follow_links=True):
        ...
        if not args.tile_diffusion:
            lq_resized = auto_resize(lq, 512)
        else:
            lq_resized = auto_resize(lq, args.tile_diffusion_size)

        x = lq_resized.resize(
            tuple((s//64+1)*64 for s in lq_resized.size), Image.LANCZOS
        )
        x = np.array(x)
        ...
        for i in range(args.repeat_times):
            ...
            preds = process(
                ...
                tile_diffusion=args.tile_diffusion, tile_diffusion_size=args.tile_diffusion_size, tile_diffusion_stride=args.tile_diffusion_stride,
                tile_vae=args.tile_vae, vae_decoder_tile_size=args.vae_decoder_tile_size, vae_encoder_tile_size=args.vae_encoder_tile_size
            )
            pred = preds[0]
